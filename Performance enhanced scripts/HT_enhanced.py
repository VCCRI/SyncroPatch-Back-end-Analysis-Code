...
def high_throughput(parent_dir, plate_name, success_qc_dir, rsquare, variant_name_file, summary_voltage, total_sweeps,
                    analysis_type):
    total_tic = time.time()
    ...
    if analysis_type == 'ssDeact CD -50 250/500':
        [summary_250ms_filename, no_well_summary_250ms_filename] = prompt_user(summary_250ms_filename_prompt, 'file',
                                                                               output_parent_dir, 1)
        [summary_500ms_filename, no_well_summary_500ms_filename] = prompt_user(summary_500ms_filename_prompt, 'file',
                                                                               output_parent_dir, 1)
        ...
        [summary_filename, no_well_summary_filename] = prompt_user(summary_filename_prompt, 'file', output_parent_dir,
                                                                   1)
        ...
        [summary_neg_50_CD_filename, no_well_summary_neg_50_CD_filename] = prompt_user(summary_neg_50_filename_prompt,
                                                                                       'file', output_parent_dir, 1)
        ...
        [summary_neg_120_CD_filename, no_well_summary_neg_120_CD_filename] = prompt_user(
            summary_neg_120_filename_prompt, 'file', output_parent_dir, 1)
        ...
        # summary_filename = 'summary_CD_neg_50_neg_120_ratio_test_pandas.csv'
        # no_well_summary_filename = 'summary_CD_neg_50_neg_120_ratio_test_pandas_no_wellID.csv'
        [summary_neg_40_CD_filename, no_well_summary_neg_40_CD_filename] = prompt_user(summary_neg_40_filename_prompt,
                                                                                       'file', output_parent_dir, 1)
        # summary_neg_50_CD_filename = 'summary_CD_neg_50_test_pandas.csv'
        # no_well_summary_neg_50_CD_filename = 'summary_CD_neg_50_test_pandas_no_wellID.csv'

        [summary_neg_110_CD_filename, no_well_summary_neg_110_CD_filename] = prompt_user(
            summary_neg_110_filename_prompt, 'file', output_parent_dir, 1)
        # summary_neg_120_CD_filename = 'summary_CD_neg_120_test_pandas.csv'
        # no_well_summary_neg_120_CD_filename = 'summary_CD_neg_120_test_pandas_no_wellID.csv'
        ...
        [V05_summary_filename, V05_no_well_summary_filename] = prompt_user(V05_summary_filename_prompt, 'file',
                                                                           output_parent_dir, 1)
        [DG_summary_filename, DG_no_well_summary_filename] = prompt_user(DG_summary_filename_prompt, 'file',
                                                                         output_parent_dir, 1)
        [k_summary_filename, k_no_well_summary_filename] = prompt_user(k_summary_filename_prompt, 'file',
                                                                       output_parent_dir, 1)
        [z_summary_filename, z_no_well_summary_filename] = prompt_user(z_summary_filename_prompt, 'file',
                                                                       output_parent_dir, 1)
        [CD_array_filename, CD_array_no_well_filename] = prompt_user(CD_array_filename_prompt, 'file',
                                                                     output_parent_dir, 1)
    ...
    for variants in range(0, (len(SatMutVariantNames))):
        ...
        for data_file in range(0, len(variant_files)):
            ...
            if analysis_type == 'ssDeact Fit':
                ...
                [current_density_ratio_250,
                 current_density_ratio_500] = ssDeact_analyse_CD_neg_50mV_250ms_500ms_peak_ratio(
                    os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID,
                    os.path.join(output_plots, variant), int(num_sweeps), variant, 3, total_sweeps, var_amp_thresh)
                ...
                # current_density_ratio = ssDeact_analyse_CD(os.path.join(cwd_path, parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID, os.path.join(output_plots, variant), int(num_sweeps), variant, 3, total_sweeps)
                [current_density_ratio_250,
                 current_density_ratio_500] = ssDeact_analyse_CD_neg_40mV_250ms_500ms_peak_ratio(
                    os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID,
                    os.path.join(output_plots, variant), int(num_sweeps), variant, 3, total_sweeps, var_amp_thresh)
                ...
                [current_density_ratio, current_density_neg_50,
                 current_density_neg_120] = ssDeact_analyse_CD_neg_50mV_neg_120mV_peak_ratio(
                    os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID,
                ...
                [current_density_ratio, current_density_neg_40,
                 current_density_neg_110] = ssDeact_analyse_CD_neg_40mV_neg_110mV_peak_ratio(
                    os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID,
                    os.path.join(output_plots, variant), int(num_sweeps), variant, 3, total_sweeps, 'neg_40_amp_thresh',
                    'neg_110_amp_thresh')
                # current_density_ratio = ssDeact_analyse_CD_neg_50mV(os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID, os.path.join(output_plots, variant), int(num_sweeps), variant, 3, total_sweeps)
                ...
                [current_density] = ssDeact_analyse_CD_neg_50mV(
                    os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID,
                    os.path.join(output_plots, variant), int(num_sweeps), variant, 3, total_sweeps)
                if current_density != 'N/A':
                    variant_summary_table = np.append(variant_summary_table, current_density)
                    variant_summary_wells = np.append(variant_summary_wells, wellID)
            elif analysis_type == 'ssDeact CD -40':
                [current_density] = ssDeact_analyse_CD_neg_40mV(
                    os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID,
                    os.path.join(output_plots, variant), int(num_sweeps), variant, 3, total_sweeps)
                if current_density != 'N/A':
                    variant_summary_table = np.append(variant_summary_table, current_density)
                ...
                [pos40mVCD, returnV05, returnDG, returnK, returnz] = ssAct_fit_py(
                    os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]),
        ...
        if analysis_type == 'Onset':
            ...
            if variant != 'neg_ctrl':
                [summary_table, no_well_summary_table] = append_summary_results(variant_summary_table,
                                                                                ...
                                                                                no_well_summary_table, [],
                                                                                analysis_type)
                [summary_table_neg_40, no_well_summary_table_neg_40] = append_summary_results(
                    variant_summary_neg_40_table, variant_summary_neg_40_wells, summary_table_neg_40,
                    no_well_summary_table_neg_40, [], analysis_type)
                [summary_table_neg_110, no_well_summary_table_neg_110] = append_summary_results(
                    variant_summary_neg_110_table, variant_summary_neg_110_wells, summary_table_neg_110,
                    no_well_summary_table_neg_110, [], analysis_type)
