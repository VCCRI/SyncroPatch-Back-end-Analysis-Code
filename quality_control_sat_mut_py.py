    ...
    for well in range(0, num_wells):
        ...
        for sweep in range(start, end_limit, interval):
            ...
            if numpy_pandas == 'numpy':
                ...
                if not float(capacitance_parameter_upper) >= float(QC_data[well, capacitance_index]) >= float(capacitance_parameter_lower):
                    '''
                    print(wellID)
                    print(sweep)
...
def main():
    ...
    quality_control_sat_mut_py(os.path.join('Z://', 'Functional_Genomics_Syncropatch', 'KCNH2', 'Tris-HCl'),
                               ...
                               'parameters_16122021_AN_hERG_ssDeact_3s_AN_14.07.31.csv',
                               os.path.join('variant names', '16122021_AN.txt'), 18, 'ssDeact', 'pandas')
