...
from ssDeact_analyse_CD import ssDeact_analyse_CD
...
def automate_high_throughput(parent_dir, plate_name, success_qc_dir, rsquare, variant_name_file, summary_voltage, total_sweeps, analysis_type, output_plots, output_dir, sweep_length):

    total_tic = time.time()

    # Create the parent results directory if it doesnt exist and the directory that encompasses the results for this analysis
    if analysis_type == 'ssDeact Fit' or analysis_type == 'ssDeact CD -50 250/500' or analysis_type == 'ssDeact CD -50/-120' or analysis_type == 'ssDeact CD -50':
    ...
    #[summary_filename, no_well_summary_filename] = prompt_user(summary_filename_prompt, 'file', output_parent_dir, 1)
        ...
        if analysis_type == 'ssDeact Fit':
            summary_filename = 'summary_tau_weighted_sweep_neg_50mV_no_series_QC_neg_120_filtering.csv'
            no_well_summary_filename = 'summary_tau_weighted_sweep_neg_50mV_no_series_QC_neg_120_filtering_no_wellID.csv'
        elif analysis_type == 'Onset':
            summary_filename = 'summary_tau_sweep_0mV.csv'
            no_well_summary_filename = 'summary_tau_sweep_0mV_no_wellID.csv'
        elif analysis_type == 'ssAct':
            summary_filename = 'summary_current_density_no_series_QC_sweep_40mV.csv'
            no_well_summary_filename = 'summary_current_density_no_series_QC_sweep_40mV_no_wellID.csv'
        elif analysis_type == 'ssDeact CD -50/-120':
            #summary_filename = 'summary_current_density_neg_120mV_on_neg_50mV.csv'
            #no_well_summary_filename = 'summary_current_density_neg_120mV_on_neg_50mV_no_wellID.csv'
            summary_filename = 'summary_neg_50_neg_120_CD_ratio_no_series_QC.csv'
            no_well_summary_filename = 'summary_neg_50_neg_120_CD_ratio_no_series_QC_no_wellID.csv'

            summary_neg_50_CD_filename = 'summary_CD_neg_50_no_series_QC.csv'
            no_well_summary_neg_50_CD_filename = 'summary_CD_neg_50_no_series_QC_no_wellID.csv'

            summary_neg_120_CD_filename = 'summary_CD_neg_120_no_series_QC.csv'
            no_well_summary_neg_120_CD_filename = 'summary_CD_neg_120_no_series_QC_no_wellID.csv'

        elif analysis_type == 'ssDeact CD -50 250/500':
            #summary_filename = 'summary_current_density_neg_120mV_on_neg_50mV.csv'
            #no_well_summary_filename = 'summary_current_density_neg_120mV_on_neg_50mV_no_wellID.csv'
            summary_250ms_filename = 'summary_current_density_neg_50mV_250ms_peak_200pA_no_series_QC.csv'
            no_well_summary_250ms_filename = 'summary_current_density_neg_250ms_peak_200pA_no_series_QC_no_wellID.csv'

            summary_500ms_filename = 'summary_current_density_neg_50mV_500ms_peak_200pA_no_series_QC.csv'
            no_well_summary_500ms_filename = 'summary_current_density_neg_500ms_peak_200pA_no_series_QC_no_wellID.csv'
        elif analysis_type == 'ssDeact CD -50':
            summary_filename = 'summary_neg_50_CD_no_series_QC_no_raw_current_threshold_QC.csv'
            no_well_summary_filename = 'summary_neg_50_CD_no_series_QC_no_raw_current_threshold_QC_no_wellID.csv'
    ...
    if analysis_type == 'ssAct':
        ...
        '''
        V05_summary_filename = 'V05_summary_no_series_QC.csv'
        V05_no_well_summary_filename = 'V05_summary_no_series_QC_no_wellID.csv'
        DG_summary_filename = 'DG_summary_no_series_QC.csv'
        DG_no_well_summary_filename = 'DG_summary_no_series_QC_no_wellID.csv'
        k_summary_filename = 'k_summary_no_series_QC.csv'
        k_no_well_summary_filename = 'k_summary_no_series_QC_no_wellID.csv'
        z_summary_filename = 'z_summary_no_series_QC.csv'
        z_no_well_summary_filename = 'z_summary_no_series_QC_no_wellID.csv'
    ...
    #output_plots = 'plots CD -50mV 250mspeak_peak 100pA current_thresh'

    '''
    if analysis_type != 'ssDeact CD -50' and analysis_type != 'ssDeact CD -50/120':
        ...
        #output_dir = 'results CD -50mV 250mspeak_peak 100pA current_thresh'
    '''
    ...
    if analysis_type != 'ssDeact CD -50 250/500' and analysis_type != 'ssDeact CD -50/-120' and analysis_type != 'ssDeact CD -50':
        ...
        if analysis_type == 'ssDeact CD -50/-120':
            ...
            no_well_summary_summary_neg_120_CD_filename = os.path.join(output_parent_dir,
                                                                       no_well_summary_neg_120_CD_filename)
        elif analysis_type == 'ssDeact CD -50 250/500':
            output_plots = os.path.join(output_parent_dir, output_plots)
            summary_250ms_filename = os.path.join(output_parent_dir, summary_250ms_filename)
            no_well_summary_250ms_filename = os.path.join(output_parent_dir, no_well_summary_250ms_filename)
            summary_500ms_filename = os.path.join(output_parent_dir, summary_500ms_filename)
            no_well_summary_500ms_filename = os.path.join(output_parent_dir, no_well_summary_500ms_filename)
        elif analysis_type == 'ssDeact CD -50':
            summary_filename = os.path.join(output_parent_dir, summary_filename)
            no_well_summary_filename = os.path.join(output_parent_dir, no_well_summary_filename)
            output_plots = os.path.join(output_parent_dir, output_plots)
    ...
    if analysis_type == 'ssAct':
        V05_summary_filename = os.path.join(output_parent_dir, V05_summary_filename)
        V05_no_well_summary_filename = os.path.join(output_parent_dir, V05_no_well_summary_filename)
        DG_summary_filename = os.path.join(output_parent_dir, DG_summary_filename)
        DG_no_well_summary_filename = os.path.join(output_parent_dir, DG_no_well_summary_filename)
        k_summary_filename = os.path.join(output_parent_dir, k_summary_filename)
        k_no_well_summary_filename = os.path.join(output_parent_dir, k_no_well_summary_filename)
        z_summary_filename = os.path.join(output_parent_dir, z_summary_filename)
        z_no_well_summary_filename = os.path.join(output_parent_dir, z_no_well_summary_filename)
    ...
    for variants in range(0, (len(variant_dir_list))):
        ...
        for data_file in range(0, len(variant_files)):
            ...
            if analysis_type == 'ssDeact Fit':
                ...
                neg_50_amp_thresh = 200
                neg_120_amp_thresh = -200
                [current_density_ratio, current_density_neg_50, current_density_neg_120] = ssDeact_analyse_CD(
                    os.path.join(output_parent_dir, success_qc_dir, variant, variant_files[data_file]), wellID,
                    os.path.join(output_plots, variant), int(num_sweeps), variant, 3, total_sweeps, 'neg_50_amp_thresh',
                    'neg_120_amp_thresh')
        ...
        if analysis_type == 'Onset':
            if variant != 'neg_ctrl':
                [summary_table, no_well_summary_table] = append_summary_results(variant_summary_table,
                                                                                ...
                                                                                no_well_summary_table,
                                                                                variant_peak_table, analysis_type)
            ...
            if variant != 'neg_ctrl':
                [summary_table, no_well_summary_table] = append_summary_results(variant_summary_table,
                                                                                ...
                                                                                no_well_summary_table, [],
                                                                                analysis_type)
                [summary_table_neg_50, no_well_summary_table_neg_50] = append_summary_results(
                    variant_summary_neg_50_table, variant_summary_neg_50_wells, summary_table_neg_50,
                    no_well_summary_table_neg_50, [], analysis_type)
                [summary_table_neg_120, no_well_summary_table_neg_120] = append_summary_results(
                    variant_summary_neg_120_table, variant_summary_neg_120_wells, summary_table_neg_120,
                    no_well_summary_table_neg_120, [], analysis_type)
        elif analysis_type == 'ssDeact CD -50 250/500':
            if variant != 'neg_ctrl':
                [summary_250ms_table, no_well_summary_250ms_table] = append_summary_results(variant_summary_250ms_table,
                                                                                            variant_summary_250ms_wells,
                                                                                            summary_250ms_table,
                                                                                            no_well_summary_250ms_table,
                                                                                            [], analysis_type)
                [summary_500ms_table, no_well_summary_500ms_table] = append_summary_results(variant_summary_500ms_table,
                                                                                            variant_summary_500ms_wells,
                                                                                            summary_500ms_table,
                                                                                            no_well_summary_500ms_table,
                                                                                            [], analysis_type)
        elif analysis_type == 'ssDeact CD -50':
    ...
    if analysis_type != 'AP pre-stim' and analysis_type != 'mod FAIV' and analysis_type != 'ssDeact CD -50 250/500' and analysis_type != 'ssDeact CD -50/-120':
        # Now write the output
...
def main():
    ...
    #high_throughput(os.path.join('Z://', 'Syncropatch'), '21022020_AN', 'success_QC', 0.85, os.path.join('variant names', '21022020_AN.txt'), -150, 10, -50, 18, 'ssDeact CD')
    automate_high_throughput(os.path.join('Z://', 'Syncropatch'), '21022020_AN', 'success_QC', 0.85, os.path.join('variant names', '21022020_AN.txt'), -150, 10, -50, 18, 'ssDeact Fit')

    ## Enter your input parameters here:
    # high_throughput(parent dir, plate name, QC dir, fit rSquared value, variant filename path, start stimulus voltage, voltage stimulus sweep interval, summary sweep voltage, number of sweeps, protocol)
