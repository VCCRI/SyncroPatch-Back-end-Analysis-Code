...
def ssDeact_analyse_CD_neg_50mV_250ms_500ms_peak_ratio(input_file, wellID, result_plots, num_sweeps, variant, sweep_length, total_sweeps, amp_thresh):
    #print(wellID)
    ...
    if numpy_pandas == 'numpy':
        ...
        time_us = time_us.astype(float)
        time_secs = time_us * 1e-6

        start_time_indx_list = [i for i in range(len(list(time_secs))) if time_secs.iloc[i] >= 1.20]  # new protocol
        start_time = start_time_indx_list[0]
        end_time_indx_list = [i for i in range(len(list(time_secs))) if time_secs.iloc[i] >= (sweep_length + 1.2)]  # new protocol
        end_time = end_time_indx_list[0]
        ...
        #voltage_array = np.array(voltage_array)
        # if summary_sweep_voltage != 52:
        voltage_array = voltage_array.iloc[1::2].astype(float)
    ...
    current_density_neg_50mV_a = 'N/A'
    current_density_neg_50mV_b = 'N/A'
    peak_neg_50mV = 'N/A'
    peak_neg_120mV = 'N/A'
    cap_neg_50mV = 'N/A'
    cap_neg_120mV = 'N/A'
    ...
    for sweep in range(1, (2*num_sweeps), 2):
        if numpy_pandas == 'numpy':
            ...
            sweep_voltage = voltage_array.iloc[int(sweep / 2)]
            #print(sweep_voltage)
        ...
        if sweep_voltage == -50:
            ...
            current_density_neg_50mV_a = max_cd_peak_a / capacitance
            ...
            if numpy_pandas == 'numpy':
                ...
                cd_250_sweep_indx = [i for i in range(0, len(list(sweepData))) if (time_secs[i] >= 0.25 and time_secs[i] <= 2)]
                cd_250_data = sweepData[cd_250_sweep_indx]
                cd_250_time_data = time_secs[cd_250_sweep_indx]

                max_cd_peak_250 = max(cd_250_data)
                max_cd_peak_250_indx = [i for i in range(0, len(list(cd_250_data))) if (cd_250_data[i] == max_cd_peak_250)]
                if max_cd_peak_250_indx[0] >= 5:
                    max_cd_peak_250_array = cd_250_data[max_cd_peak_250_indx[0] - 5:max_cd_peak_250_indx[0] + 6]

                    max_cd_peak_250 = np.mean(max_cd_peak_250_array)
            else:
                loc_max_cd = [i for i in range(0, len(list(cd_data))) if cd_data.iloc[i] >= max_cd_peak_a]
                loc_max_cd_a = cd_time_data.iloc[loc_max_cd[0]]

                cd_250_sweep_indx = [i for i in range(0, len(list(sweepData))) if (time_secs.iloc[i] >= 0.25 and time_secs.iloc[i] <= 2)]
                cd_250_data = sweepData.iloc[cd_250_sweep_indx]
                cd_250_time_data = time_secs.iloc[cd_250_sweep_indx]

                max_cd_peak_250 = max(cd_250_data)
                max_cd_peak_250_indx = [i for i in range(0, len(list(cd_250_data))) if
                                        (cd_250_data.iloc[i] == max_cd_peak_250)]
                if max_cd_peak_250_indx[0] >= 5:
                    max_cd_peak_250_array = cd_250_data.iloc[max_cd_peak_250_indx[0] - 5:max_cd_peak_250_indx[0] + 6]

                    max_cd_peak_250 = np.mean(max_cd_peak_250_array)
            current_density_neg_50mV_250 = max_cd_peak_250 / capacitance

            if numpy_pandas == 'numpy':
                cd_500_sweep_indx = [i for i in range(0, len(list(sweepData))) if (time_secs[i] >= 0.5 and time_secs[i] <= 2)]
                cd_500_data = sweepData[cd_500_sweep_indx]
                cd_500_time_data = time_secs[cd_500_sweep_indx]

                max_cd_peak_500 = max(cd_500_data)
                max_cd_peak_500_indx = [i for i in range(0, len(list(cd_500_data))) if (cd_500_data[i] == max_cd_peak_500)]

                if max_cd_peak_500_indx[0] >= 5:
                    max_cd_peak_500_array = cd_500_data[max_cd_peak_500_indx[0] - 5:max_cd_peak_500_indx[0] + 6]

                    max_cd_peak_500 = np.mean(max_cd_peak_500_array)
            else:
                cd_500_sweep_indx = [i for i in range(0, len(list(sweepData))) if (time_secs.iloc[i] >= 0.5 and time_secs.iloc[i] <= 2)]
                cd_500_data = sweepData.iloc[cd_500_sweep_indx]
                cd_500_time_data = time_secs.iloc[cd_500_sweep_indx]

                max_cd_peak_500 = max(cd_500_data)
                max_cd_peak_500_indx = [i for i in range(0, len(list(cd_500_data))) if
                                        (cd_500_data.iloc[i] == max_cd_peak_500)]

                if max_cd_peak_500_indx[0] >= 5:
                    max_cd_peak_500_array = cd_500_data.iloc[max_cd_peak_500_indx[0] - 5:max_cd_peak_500_indx[0] + 6]

                    max_cd_peak_500 = np.mean(max_cd_peak_500_array)

            current_density_neg_50mV_500 = max_cd_peak_500 / capacitance


            if numpy_pandas == 'numpy':
                loc_max_cd = [i for i in range(0, len(list(cd_250_data))) if cd_250_data[i] >= max_cd_peak_250]
                loc_max_cd_250 = cd_250_time_data[loc_max_cd[0]]

                loc_max_cd = [i for i in range(0, len(list(cd_500_data))) if cd_500_data[i] >= max_cd_peak_500]
                loc_max_cd_500 = cd_500_time_data[loc_max_cd[0]]
            else:
                loc_max_cd = [i for i in range(0, len(list(cd_250_data))) if cd_250_data.iloc[i] >= max_cd_peak_250]
                loc_max_cd_250 = cd_250_time_data.iloc[loc_max_cd[0]]

                loc_max_cd = [i for i in range(0, len(list(cd_500_data))) if cd_500_data.iloc[i] >= max_cd_peak_500]
                loc_max_cd_500 = cd_500_time_data.iloc[loc_max_cd[0]]
    ...
    plt.close('all')
    if current_density_neg_50mV_a != 'N/A' and current_density_neg_50mV_250 != 'N/A':
        current_density_ratio_250 = current_density_neg_50mV_250 / current_density_neg_50mV_a
        current_density_ratio_500 = current_density_neg_50mV_500 / current_density_neg_50mV_a
    else:
        current_density_ratio_250 = 'N/A'
        current_density_ratio_500 = 'N/A'

    if ret_max_current == 'no':
        current_density_ratio_250 = 'N/A'
        current_density_ratio_500 = 'N/A'
    ...
    return [current_density_ratio_250, current_density_ratio_500]
