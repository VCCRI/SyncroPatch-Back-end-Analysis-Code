        ...
        for f in range(0, len(result_files)):
            ...
            for r in range(1, np.shape(data)[0]):
                ...
                if row_voltage == summary_voltage:
                    ...
                    variant_summary_table = np.append(variant_summary_table, row_data[5])
                    variant_summary_wells = np.append(variant_summary_wells, wellID)
                    variant_peak_table = np.append(variant_peak_table, row_data[2])
...
def summarise_onset(parent_dir, plate_name, results_dir, summary_voltage, variant_name_file):

    output_dir = os.path.join(parent_dir, 'Data Analysis Results Onset Inact', plate_name)
    full_results_dir = os.path.join(parent_dir, 'Data Analysis Results Onset Inact', plate_name, results_dir)
    variant_name_file = os.path.join(parent_dir, variant_name_file)


    if summary_voltage == 'all':
        summary_filename_prompt = 'Please enter the base name you would like to call each of the .csv files which store the summaries of the tau values (will be tagged with summary voltage automatically): '
        # [summary_filename, no_well_summary_filename] = prompt_user(summary_filename_prompt, 'file', output_parent_dir, 1)
        summary_basename = 'summary_tau_no_series'
        no_well_summary_basename = 'summary_tau_no_series_no_wellID'

        for volt in range(60, -60, -10):
        ...
        # [summary_filename, no_well_summary_filename] = prompt_user(summary_filename_prompt, 'file', output_parent_dir, 1)
        summary_filename = 'summary_tau_sweep_10mV_no_series_QC.csv'
        no_well_summary_filename = 'summary_tau_sweep_10mV_no_series_QC_no_wellID.csv'
        summary_filename = os.path.join(output_dir, summary_filename)
        no_well_summary_filename = os.path.join(output_dir, no_well_summary_filename)
        process_file(variant_name_file, full_results_dir, summary_voltage, summary_filename, no_well_summary_filename)

def main():
    summarise_onset(os.path.join('Z://', 'Syncropatch'), '02072020_AN', 'results no series QC', 'all', os.path.join('variant names', '02072020_AN.txt'))
