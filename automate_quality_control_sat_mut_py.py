...
def automate_quality_control_sat_mut_py(parent_dir, data_dir, plate_name, qc_file, variant_name_file, total_sweeps, analysis, seal_parameter, capacitance_parameter_upper, capacitance_parameter_lower, access_parameter, peak_current_parameter, output_dir, qc_stats_file, numpy_pandas):
#parent_dir, data_dir, plate_name
    ...
    for well in range(0, num_wells):
        ...
        if numpy_pandas == 'numpy':
            ...
            # pandas
            rawWellData = pd.read_csv(current_file[0], sep='\t', low_memory=False, usecols=[i for i in range((2 * total_sweeps) + 2)], header=None)
        ...
        for sweep in range(start, end_limit, interval):
            ...
            if analysis == 'AP pre-stim':
                ...
                if numpy_pandas == 'numpy':
                    ...
                    # pandas
                    if analysis == 'ssAct' or analysis == 'ssDeact':
                        result = pd.concat((pd.DataFrame([QC_data.iloc[well, capacitance_index]]), rawWellData.iloc[:, sweep]), axis=0)
                        v_result = pd.concat(
